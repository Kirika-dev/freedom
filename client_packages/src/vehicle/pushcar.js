...
class PushBrokenDownClient {
    ...
    canPushVehicle() {
        ...
        const idVehicle = mp.game.vehicle.getClosestVehicle(player.position.x, player.position.y, player.position.z, 10, 0, 70)
        if (idVehicle === false)
            return false;
        ...
        if (!Vehicle.isOnAllWheels()) // Make sure its on the ground
            return false;

        if(this.config.AllowVehicleClass !== null && this.config.AllowVehicleClass !== false && this.config.AllowVehicleClass.indexOf(vehClass) === -1)
            return false;

        if(this.config.EngineProtection === true && Vehicle.engine === true)
            return false;

        if(this.config.LockHandbrakeProtection === true && Vehicle.locked === true)
            return false;

        if(this.config.VehicleHealthProtection !== false && Vehicle.getEngineHealth() >= this.config.VehicleHealthProtection)
            return false;
		...
		const engineState = Vehicle.getIsEngineRunning();
		if (this.config.State == true) {
			if (mp.keys.isDown(32) === true) {
				this.config.State = false;
				mp.events.callRemote("server::veh:exitpushcar");
			}
		}
        if ((distanceFront <= 2 || distanceBack <= 2) && ((vehicleSize.size.lengthY * vehicleSize.size.lengthX) <= this.config.MaxVehicleCubedSize) && engineState == false) {
            if (this.config.State == true) {
                if (!this.PlayerPushing && !player.isInAnyVehicle(false)) {
                    ...
                    if (Vehicle.IsInFront) {
                        player.attachTo(Vehicle.handle, 6286, 0.0, Vehicle.vehicleSize.size.max.y + 0.35, Vehicle.vehicleSize.size.z + 0.95, 0.0, 0.0, 180.0, false, false, false, false, 0, true)
                    } else {
                        player.attachTo(Vehicle.handle, 6286, 0.0, Vehicle.vehicleSize.size.min.y - 0.6, Vehicle.vehicleSize.size.z + 0.95, 0.0, 0.0, 0.0, false, false, false, false, 0, true)
                    ...
                    setTimeout(() => {
                        if(this.config.PushEventName !== false) mp.events.callRemote(this.config.PushEventName, Vehicle, player)
...
const BrkCar = new PushBrokenDownClient();
mp.events.add("client::veh:pushcarstate", (a) => {
	BrkCar.config.State = a;
});