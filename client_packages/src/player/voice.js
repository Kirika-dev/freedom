...
mp.keys.bind(VoiceChatKey, true, function() {
    ...
    if(mp.gui.cursor.visible != false && ENABLE_VOICE_WITH_CURSOR == false) return ;

    let time = Date.now();
    
    
	//if(localplayer.getVariable("muteTime") > 0) return showAlert("alert-red", "У Вас отключен голосовой чат. Оставшееся время: "+localplayer.getVariable("muteTime")+" мин.");
