    ...
    if (isAdmin == true) {
        ...
        if (player === undefined || player.handle === undefined || !player.handle) {} else {
            if (player.getType() === 4 && player != global.localplayer) {
                mp.game.graphics.drawText(player.name + ' (' + player.remoteId + ')', [0.25, 0.25], { font: 0, color: [255,255,255,255], scale: [1, 0.50], outline: false });
            }
        }
    }
                ...
                if (calculateDistance(playerPos, _player.position) < 15.0) {
                    if (_player.getVariable('INVISIBLE') != true && _player.getVariable('HideNick') != true) {
                        ...
                        if (tagLabelPool[_player.remoteId] === undefined || reupdateTagLabel[_player.remoteId] === undefined || new Date().getTime() - reupdateTagLabel[_player.remoteId] > 500) {
                            ...
                            var text = void 0;
							var bGender = true;
							if (_player.model != 1885233650) bGender = false;
							var namegender = bGender ? 'Незнакомец ' : 'Heзнaкoмкa '
                            if (_player.getVariable('IS_MASK') == true) {
                                if (isAdmin === true) text = '\u0418\u0433\u0440\u043E\u043A \u0432 \u043C\u0430\u0441\u043A\u0435: ' + _playerName + ' [' + _player.remoteId + passportText + ']';else text = namegender + '[' + _player.remoteId + ']';
                            } else {
                                if (isAdmin === true || mp.storage.data.friends[_player.name] !== undefined) text = _playerName + ' [' + _player.remoteId + passportText + ']';else text = namegender + '[' + _player.remoteId + ']';;
                            }
                        ...
                        if (label !== undefined) {
							...
							drawPlayerTag(_player, x, y, '\n #' + _player.getVariable('UID'), [255,255,255,150]);
...
function drawPlayerTag(player, x, y, displayname, color) {
	global.drawText(displayname, x, y, 0.25, color[0], color[1], color[2] , color[3])
...
function drawPlayerVoiceIcon(player, x, y) {
	if (player.isVoiceActive) drawVoiceSprite("mpleaderboard", 'leaderboard_audio_3', [0.6, 0.6], 0, [255, 255, 255, 255], x, y - 0.02 * 0.7);else if (player.getVariable('voice.muted') == true) drawVoiceSprite("mpleaderboard", 'leaderboard_audio_mute', [0.6, 0.6], 0, [255, 0, 0, 255], x, y - 0.02 * 0.7);
}
function drawPlayercrown(player, x, y) {
    drawVoiceSprite("commonmenu", 'mp_hostcrown', [0.6, 0.6], 0, [255, 255, 255, 255], x, y - 0.01);
}
function drawPlayerdeath(player, x, y) {
    drawVoiceSprite("commonmenutu", 'deathmatch', [0.6, 0.6], 0, [255, 0, 0, 255], x, y - 0.01);
}
