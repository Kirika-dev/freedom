var tablet = new Vue({
	...
	methods: {	    
		ChangePromoPage: function(page){
			switch(page){
				case 'using':
					if(this.promocodePage == 'using'){
						this.promocodePage = ""
						return;
					}
					this.promocodePage = page
				return;
				case 'history':
					if(this.promocodePage == 'history'){
						this.promocodePage = ""
						return;
					}
					this.promocodePage = page
		...
		AddPromocodesHistory: function(json){
			this.promocods = JSON.parse(json)
		},
		usePromocode: function(){
			mp.trigger('usePromo', this.promocod)
		},
		sel: function (index) {
			this.myselect = index;
		},
		...
		driftMode: function(){
			...
			mp.trigger('driftMode', this.drift);
		},
		showpos: function(){
			this.pos = !this.pos;
			mp.trigger('showPOS');
