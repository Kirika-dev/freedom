class CameraRotator {
	start(camera, basePosition, lookAtPosition, offsetVector, heading, fov = undefined, sf = 0) {
		this.camera = camera;
	...
	onMouseMove(dX, dY) {
		...
		if (global.autoshop != null) {
			global.autoshop.execute(`auto.mousedown=true`);
		}
		this.changePosition();
	}
...
mp.events.add("render", () => {
	...
	const safeZone = cameraRotator.getSafeZone();
	
	cameraRotator.setPoint(x, y);
	
	if (!global.modalAuto && mp.game.controls.isDisabledControlPressed(2, 237) && mp.gui.cursor.position[0] > safeZone && mp.gui.cursor.position[0] < mp.game.graphics.getScreenActiveResolution(100, 100).x - safeZone) {
		cameraRotator.onMouseMove(dX, dY);
	}
	else {
		if (global.autoshop != null) {
			global.autoshop.execute(`auto.mousedown=false`);
		}
	}
});
