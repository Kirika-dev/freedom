...
namespace NeptuneEVO.MoneySystem
{
    class Bank : Script
    {
        ...
        public static List<Vector3> BANKs = new List<Vector3>()
        {
            new Vector3(149.83676, -1040.7944, 29.374088),
            new Vector3(-2962.5046, 482.93707, 15.703119),
            new Vector3(314.15976, -279.07413, 54.170784),
            new Vector3(-1212.6235, -330.7521, 37.78703),
            new Vector3(1175.0422, 2706.8643, 38.09405),
            new Vector3(248.47623, 222.90514, 106.286736),
            new Vector3(-350.9935, -49.96192, 49.0426),
        };
        ...
        public static bool Change(int accountID, long amount, bool notify = true)
        {
            lock (Accounts)
            {
                ...
                MySQL.Query($"UPDATE `money` SET balance={Accounts[accountID].Balance} WHERE id={accountID}");
                if (Accounts[accountID].Type == 1 && NAPI.Player.GetPlayerFromName(Accounts[accountID].Holder) != null)
                {
                    NAPI.Task.Run(() =>
                    {
                        try
                        {
                            ...
                            {
                                if (amount > 0)
                                    BankNotify(GetPlayerByName(accountID), BankNotifyType.PayIn, amount.ToString());
                                else
                                    BankNotify(GetPlayerByName(accountID), BankNotifyType.PayOut, amount.ToString());
                            }
        ...
        public static Player GetPlayerByName(int idacc)
        {
            Player result = null; 
            foreach (Player player in NAPI.Pools.GetAllPlayers())
            {
                if (!Main.Players.ContainsKey(player)) continue;
                if (Main.Players[player].Bank == idacc)
                {
                    result = player; break;;
                }
            }
        ...
        {
            foreach (Vector3 v in BANKs)
            {
                NAPI.Blip.CreateBlip(500, v, 0.7f, 11, Main.StringToU16("Банк"), 255, 0, true, 0, 0);
            }
        }

        public static Vector3 GetNearestBANK(Player player)
        {
            Vector3 nearesetBANK = BANKs[0];
            foreach (var v in BANKs)
            {
                if (player.Position.DistanceTo(v) < player.Position.DistanceTo(nearesetBANK))
                    nearesetBANK = v;
            }
            return nearesetBANK;
        }
        ...
        public static void Update(Player Player)
        {
            NAPI.Task.Run(() =>
            {
                Trigger.PlayerEvent(Player, "UpdateBank", Get(Player.Name).Balance);
            });
        }
    ...
    class ATM : Script
    {
        ...
        public static List<Vector3> ATMs = new List<Vector3>
        {
            ...
            new Vector3(-2962.5046, 482.93707, 15.703119),
            new Vector3(314.15976, -279.07413, 54.170784),
            new Vector3(-113.00502, 6470.2393, 31.626709),
            new Vector3(-1212.6235, -330.7521, 37.78703),
            new Vector3(1175.0422, 2706.8643, 38.09405),
            new Vector3(248.47623, 222.90514, 106.286736),
            new Vector3(-350.9935, -49.96192, 49.0426),
        ...
        public void onResourceStart()
        {
            try
            {
                Log.Write("Loading ATMs...", nLog.Type.Success);
        ...
        public static void OpenATM(Player player)
        {
            ...
            Trigger.PlayerEvent(player, "setatm", acc.Bank.ToString(), player.Name, balance.ToString(), "");
            Trigger.PlayerEvent(player, "openatm");
            return;
        }
        ...
        public static void AtmBizGen(Player player)
        {
            ...
            if (acc.BizIDs.Count > 0)
            {
                ...
                {
                    BCore.Bizness biz = BCore.BizList[key];
                    string name = biz.GetName();
                    data.Add(new List<object>{ name.ToString() + $" {Bank.Accounts[biz.BankID].Balance}/{biz.GetNalog() * 24 * 7}", i });
                    i++;
                }
                Trigger.PlayerEvent(player, "atmOpenBiz", JsonConvert.SerializeObject(data), "");
            }
            else
            {
                Trigger.PlayerEvent(player, "atmOpen", "[1,0,0]");
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "У вас нет бизнеса!", 3000);
            }
        }

        [RemoteEvent("atmVal")]
        public static void PlayerEvent_ATMVAL(Player player, params object[] args)
        {
            try
            {
                ...
                {
                    case 0:
                        Trigger.PlayerEvent(player, "atmClose");
                        if (Wallet.Change(player, -Math.Abs(amount)))
                        {
                            ...
                            Trigger.PlayerEvent(player, "setbank", Bank.Accounts[acc.Bank].Balance.ToString(), "");
                        }
                        break;
                    case 1:
                    ...
                    case 2:
                        ...
                        Trigger.PlayerEvent(player,
                                "atmOpen", $"[2,'{Bank.Accounts[house.BankID].Balance}/{Convert.ToInt32(house.Price / 100 * 0.005) * 24 * 7}$','Сумма внесения наличных']");
                        break;
                    case 3:
                        int bid = player.GetData<int>("ATMBIZ");

                        BCore.Bizness biz = BCore.BizList[acc.BizIDs[bid]];
                        if (Bank.Accounts[biz.BankID].Balance + Math.Abs(amount) > biz.GetNalog() * 24 * 7)
                        {
                        ...
                        GameLog.Money($"player({Main.Players[player].UUID})", $"bank({biz.BankID})", Math.Abs(amount), $"atmBizADD");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Успешный перевод.", 3000);
                        Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[biz.BankID].Balance}/{biz.GetNalog() * 24 * 7}$','Сумма зачисления']");
                        break;
                    case 5:
                        int bids = player.GetData<int>("ATMBIZ");

                        BCore.Bizness bizs = BCore.BizList[acc.BizIDs[bids]];

                        if (!Bank.Change(bizs.BankID, -Math.Abs(amount)))
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно средств.", 3000);
                            return;
                        }
                        Wallet.Change(player, +Math.Abs(amount));
                        GameLog.Money($"player({Main.Players[player].UUID})", $"bank({bizs.BankID})", Math.Abs(amount), $"atmBizREMOVE");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Успешный перевод.", 3000);
                        Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[bizs.BankID].Balance}/{bizs.GetNalog() * 24 * 7}$','Сумма вывода']");
                        break;
                    case 6:
                        var apartament = Houses.HouseManager.GetApart(player, true);
                        if (apartament == null) return;

                        maxMoney = Convert.ToInt32(apartament.Price / 100 * 0.005) * 24 * 7;
                        if (Bank.Accounts[apartament.BankID].Balance + Math.Abs(amount) > maxMoney)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Невозможно перевести столько средств на счет дома.", 3000);
                            return;
                        }
                        ...
                        Bank.Change(apartament.BankID, +Math.Abs(amount));
                        GameLog.Money($"player({Main.Players[player].UUID})", $"bank({apartament.BankID})", Math.Abs(amount), $"atmHouse");
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, "Успешный перевод.", 3000);
                        Trigger.PlayerEvent(player,
                                "atmOpen", $"[2,'{Bank.Accounts[apartament.BankID].Balance}/{Convert.ToInt32(apartament.Price / 100 * 0.005) * 24 * 7}$','Сумма внесения наличных']");
                        break;
                    case 4:
                        if (!Bank.Accounts.ContainsKey(amount) || amount <= 0)
                        {
                            ...
                            Trigger.PlayerEvent(player, "closeatm");
                            return;
                        }
        ...
        [RemoteEvent("atmDP")]
        public static void PlayerEvent_ATMDupe(Player Player)
        {
        ...
        [RemoteEvent("atmCB")]
        public static void PlayerEvent_ATMCB(Player player, params object[] args)
        {
            try
            {
                var acc = Main.Players[player];
                ...
                switch (type)
                {
                    case 1:
                        switch (index)
                        {
                            ...
                            case 2:
                                ...
                                Trigger.PlayerEvent(player,
                                    "atmOpen", $"[2,'{Bank.Accounts[house.BankID].Balance}/{Convert.ToInt32(house.Price / 100 * 0.005) * 24 * 7}$','Сумма внесения наличных']");
                                Trigger.PlayerEvent(player, "setatm", "Дом", $"Дом #{house.ID}", Bank.Accounts[house.BankID].Balance, "");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", index);
                                break;
                            case 3:
                                AtmBizGen(player);
                            ...
                            case 6:
                                ...
                                Trigger.PlayerEvent(player,
                                    "atmOpen", $"[2,'{Bank.Accounts[apartament.BankID].Balance}/{Convert.ToInt32(apartament.Price / 100 * 0.005) * 24 * 7}$','Сумма внесения наличных']");
                                Trigger.PlayerEvent(player, "setatm", "Квартира", $"Квартира #{apartament.ID}", Bank.Accounts[apartament.BankID].Balance, "");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", index);
                                break;
                    ...
                    case 2:
                        Trigger.PlayerEvent(player, "atmOpen", "[1,0,0]");
                        Trigger.PlayerEvent(player, "setatm", acc.Bank, player.Name, Bank.Accounts[acc.Bank].Balance, "");
                        break;
                    case 3:
                        NAPI.Data.SetEntityData(player, "ATMBIZ", index);
                        List<string> data = new List<string> { "Перевести на счёт", "Перевести со счёта" };
                        Trigger.PlayerEvent(player, "atmOpenBizp", JsonConvert.SerializeObject(data), "");
                        break;
                    case 5:
                        BCore.Bizness biz = BCore.BizList[acc.BizIDs[player.GetData<int>("ATMBIZ")]];
                        switch (index)
                        {
                            case -1:
                                Trigger.PlayerEvent(player, "atmOpen", "[1,0,0]");
                                ...
                                return;
                            case 0:
                                Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[biz.BankID].Balance}/{Convert.ToInt32(biz.GetNalog()) * 24 * 7}$','Сумма зачисления']");
                                Trigger.PlayerEvent(player, "setatm", $"{biz.BankID}", biz.GetName(), Bank.Accounts[biz.BankID].Balance, "");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", 3);
                                return;
                            case 1:
                                Trigger.PlayerEvent(player, "atmOpen", $"[2,'{Bank.Accounts[biz.BankID].Balance}/{Convert.ToInt32(biz.GetNalog()) * 24 * 7}$','Сумма вывода']");
                                Trigger.PlayerEvent(player, "setatm", $"{biz.BankID}", biz.GetName(), Bank.Accounts[biz.BankID].Balance, "");
                                NAPI.Data.SetEntityData(player, "ATMTYPE", 5);
                                return;
        ...
        [RemoteEvent("atm")]
        public static void PlayerEvent_ATM(Player player, params object[] args)
        {
            try
            {
                if (Admin.IsServerStoping)
                ...
                switch (act)
                {
                    ...
                    case 3: //put biz tax
                        ...
                        maxMoney = Convert.ToInt32(biz.Cost / 100 * 0.005) * 24 * 7;
