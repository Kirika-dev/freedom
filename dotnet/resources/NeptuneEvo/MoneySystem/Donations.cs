...
namespace NeptuneEVO.MoneySystem
{
    class Donations : Script
    {
                            ...
                            else
                            {
                                ...
                                NAPI.Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        Notify.Send(Player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вам пришли {reds} MC", 3000);
        ...
        public static void timer_donate()
        {
        ...
        public static void timer_vehicles()
        {
            try
            {
                    ...
                    case Type.Convert:
                        {
                            ...
                            int sum = amount * 100000;
                            Wallet.Change(Player, sum);
                    ...
                    case Type.PersonalPromo:
                        {
                            ...
                            {
                                Notify.Error(Player,"Неверно введены данные.", 2500);
                                return;
                            }
                            newpromo = data.ToLower();
                            var result1 = MySQL.QueryRead($"SELECT * FROM accounts");
                            if (result1 == null || result1.Rows.Count == 0)
                            {
                                Log.Write("DB CustomPromo return null result.", nLog.Type.Warn);
                                return;
                            }
                            foreach (DataRow Row in result1.Rows)
                            {
                                string custompromo = Row["custompromo"].ToString();
                                custompromo = custompromo.ToLower();
                                if (custompromo == newpromo)
                                {
                                    Notify.Error(Player, "Такой промокод уже существует.", 2500);
                                    return;
                                }
                            }
                            Main.Accounts[Player].RedBucks -= 100;
                            string json = Newtonsoft.Json.JsonConvert.SerializeObject(newpromo);
                            string json1 = Newtonsoft.Json.JsonConvert.SerializeObject(Main.Accounts[Player].CustomPromo);
                            MySQL.Query($"UPDATE accounts SET promocodes='{json}' WHERE promocodes='{json1}'");
                            Main.Accounts[Player].CustomPromo = newpromo;
                            MySQL.Query($"UPDATE accounts SET custompromo='{newpromo}' WHERE login='{Main.Accounts[Player].Login}'");
                            GameLog.Money(acc.Login, "server", 100, "donatePromo");
                            Notify.Succ(Player, $"Ваш новый промокод: {newpromo}.", 2500);
                            break;
                        }
                    case (Type)17:
                    case (Type)18:
                ...
                try
                {
                    ...
                    foreach (Vehicle veh in NAPI.Pools.GetAllVehicles())
                    {
                        if (veh.NumberPlate == num)
                        {
                            ...
                            MySQL.Query($"update `accounts` set `redbucks`={Main.Accounts[player].RedBucks} where `login`='{Main.Accounts[player].Login}'");
                            Trigger.PlayerEvent(player, "redset", Main.Accounts[player].RedBucks);
                            Notify.Send(player, NotifyType.Success, NotifyPosition.TopCenter, $"Вы сменили номер на {Utilis.VehiclesName.GetRealVehicleName(VehicleManager.Vehicles[number].Model)}", 3000);

                            Utilis.Forbes.RM_getforbes(player);
