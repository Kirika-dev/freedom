...
namespace NeptuneEVO.Core 
{
    class Phone : Script
    {
        ...
        public void Player_to_server_PhoneUpdate(Player player, params object[] args)
        {
            ...
            cmd.CommandText = $"UPDATE `phones_number` SET address_book = @address_book, call_log_phone = @call_log_phone, sms_chat_list = @sms_chat_list WHERE number = '{number_sim}'";
            cmd.Parameters.AddWithValue("@address_book", address_bock);
            cmd.Parameters.AddWithValue("@call_log_phone", call_log_phone);
            cmd.Parameters.AddWithValue("@sms_chat_list", sms_chat_list);
            MySQL.Query(cmd);
        }
        ...
        public void Player_to_server_Treatment(Player player, params object[] args)
        {
            var target_number_sim = Convert.ToInt32(args[0]);
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandText = $"UPDATE `phones_number` SET sms_chat_list = '{Convert.ToString(args[1])}' WHERE number = '{target_number_sim}'";
            cmd.Parameters.AddWithValue("@sms_chat_list", Convert.ToString(args[1]));
            MySQL.Query(cmd);
        }
