...
namespace NeptuneEVO.Core
{
    class Weapons : Script
    {
        ...
        {
            { ItemType.Pistol, 400 },
            { ItemType.CombatPistol, 450 },
            { ItemType.Pistol50, 9 },
            { ItemType.SNSPistol, 6 },
            { ItemType.HeavyPistol, 18 },
            { ItemType.VintagePistol, 7 },
            { ItemType.MarksmanPistol, 1 },
			{ ItemType.DoubleAction, 6 },
            { ItemType.Revolver, 6 },
            { ItemType.APPistol, 18 },
            { ItemType.StunGun, 0 },
            { ItemType.FlareGun, 1 },

            { ItemType.MicroSMG, 120 },
            { ItemType.MachinePistol, 130 },
            { ItemType.SMG, 150 },
            { ItemType.AssaultSMG, 150 },
            { ItemType.CombatPDW, 180 },
            { ItemType.MG, 200 },
            { ItemType.CombatMG, 140 },
            { ItemType.Gusenberg, 300 },
            { ItemType.MiniSMG, 200 },
                ...
                var wType = (ItemType)player.GetData<object>("LastActiveWeap");
                    ...
                    if (activeWeapon != null)
                    {
                        if (WeaponsAmmoTypes.ContainsKey(activeWeapon.Type))
                        {
                            ...
                            if (activeWeapon != null)
                            {
                                nInventory.Remove(player, activeWeapon.Type, 1);
                            }
                        }
        ...
        public static void RemoteEvent_playerReload(Player player, int hash, int ammoInClip)
        {
            try
            {
                ...
                Trigger.PlayerEvent(player, "wgive", hash, ammo, true, true);
            }
        ...
        public static void RemoteEvent_takeoffWeapon(Player player, int hash, int ammoInClip) // вызывается, если оружие убрали серверно
        {
            ...
            catch (Exception e) { Log.Write($"takeoffWeapon: ({player.Name})" + e.Message, nLog.Type.Error); }
        }
        ...
        public static void RemoteEvent_changeWeapon(Player player, int key)
        {
            try
            {
                ...
                switch (key)
                {
                    ...
                    case 3:
                        {
							var wItem = nInventory.Items[UUID].FirstOrDefault(i => nInventory.WeaponsItems.Contains(i.Type) && WeaponsAmmoTypes[i.Type] == ItemType.RiflesAmmo);
                            if (wItem != null) Items.onUse(player, wItem, -1); 
                        }
                        return;
					case 4:
                        {	
						var wItem = nInventory.Items[UUID].FirstOrDefault(i => i.Type == ItemType.HealthKit);
                            if (wItem != null) Items.onUse(player, wItem, -1);
						}
                        return;	
                }
    ...
    class ArmedBody : Script
    {
        ...
        public static void InitWeapons(Player player)
        {
            if (!Main.Players.ContainsKey(player)) return;

            foreach(nItem weapon in nInventory.Items[Main.Players[player].UUID])
            {
                if (nInventory.WeaponsItems.Contains(weapon.Type))
                {
                    WeaponHash ha = (WeaponHash)Weapons.GetHash(weapon.Type.ToString());
                    //CreateWeaponProp(player, ha);
                }
            }
