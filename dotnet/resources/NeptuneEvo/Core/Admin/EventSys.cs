...
namespace NeptuneEVO.Core
{
    class EventSys : Script
    {
        ...
        {
            AdminEvent.RewardLimit = 100000000;
        }
            ...
            if (Main.Players.ContainsKey(player))
            {
                ...
                if (AdminEvent.EventState == 0)
                {
                    if (eventname.Length < 50)
                    {
                        ...
                        AddAdminEventLog();
                        NAPI.Chat.SendChatMessageToAll("!{#A87C33}Уважаемые жители нашего штата, скоро начнётся мероприятие '" + eventname + "'!");
                    ...
                    if (AdminEvent.EventState == 1)
                    {
                        if (!AdminEvent.EventMembers.Contains(player))
                        {
                            if (AdminEvent.MembersLimit == 0 || AdminEvent.EventMembers.Count < AdminEvent.MembersLimit)
                            {
                                ...
                                player.SetData<Vector3>("MpPosition", player.Position);
                ...
                if (AdminEvent.EventState == 1)
                {
                    ...
                    if (target != null)
                    {
                        if (AdminEvent.EventMembers.Contains(target))
                        {
                            AdminEvent.Admin = player;
                            target.Dimension = 0;
                            if (target.HasData("MpPosition"))
                            {
                                NAPI.Entity.SetEntityPosition(target, target.GetData<Vector3>("MpPosition"));
                                target.ResetData("MpPosition");
                            }
        ...
        private void CloseAdminEvent(Player winner, uint reward)
        {
            ...
            foreach (Player p in NAPI.Pools.GetAllPlayers())
            {
                ...
                if (!p.HasData("MpPosition")) continue;
                NAPI.Entity.SetEntityPosition(p, p.GetData<Vector3>("MpPosition"));
                p.ResetData("MpPosition");
                p.Dimension = 0;
                p.Health = 100;
            }
