    ...
    class AirVehicles : Script
    {
        ...
        public static void Remove(string number)
        {
            try
            {
                ...
                cmd.CommandText = "DELETE FROM `airvehicles` WHERE number=@number";
        ...
        public static void OpenMenu(Player player)
        {
            ...
            foreach (var v in getAllAirVehicles(player.Name).Values)
            {
                ...
                menuItem.Text = $"{Utilis.VehiclesName.GetRealVehicleName(v.Model)} <br> Номер: {v.Number} <br>";
                menu.Add(menuItem);
            }
        ...
        private static void callback_selectedair(Player player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            ...
            switch (item.ID)
            {
                case "sell":
                    ...
                    MenuManager.Close(player);
                    Trigger.PlayerEvent(player, "openDialog", "AIR_SELL_TOGOV", $"Вы действительно хотите продать государству {Utilis.VehiclesName.GetRealVehicleName(vData.Model)} ({menu.Items[0].Text}) за ${price}?");
                    return;
                case "evac":
        ...
        public static void SpawnCar(Player player, int key)
        {
            try
            {
                ...
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вас ожидает {Utilis.VehiclesName.GetRealVehicleName(Airs[number].Model)}", 3000);
