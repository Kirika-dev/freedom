...
namespace NeptuneEVO.Core
{
    public class GameLog
    {
        ...
        private static string DB = MySQL_Logs._dbName;
        ...
        {
            //Discord.Bot.SendLog("STOCK", Frac, Uuid, Type, Amount, In);
            if (thread == null) return;
            queue.Enqueue(string.Format(
        ...
        {
            if (Player != null)
            //Discord.Bot.SendLog("ADMIN", Admin, Action, Player);
            if (thread == null) return;
            queue.Enqueue(string.Format(
        ...
        {
            //Discord.Bot.SendLog("MONEY", From, To, Amount, Comment);
            if (thread == null) return;
            queue.Enqueue(string.Format(
        ...
        {
            //Discord.Bot.SendLog("BAN", Admin, Player, Until, Reason, isHard);
            if (thread == null) return;
            queue.Enqueue(string.Format(
        ...
        {
            //Discord.Bot.SendLog("CONN", Name, Uuid, SClub, Hwid, Id, ip);
        ...
        {
            //Discord.Bot.SendLog("EVENTS", AdmName, EventName, MembersLimit, Started);
            if (thread == null) return;
            queue.Enqueue(string.Format(
