...
namespace NeptuneEVO.Core
{
    public class DrivingSchool : Script
    {
        ...
        private static List<int> LicPrices = new List<int>() { 5400, 15700, 23000, 75500, 100000, 130000 };
        public static Vector3 enterSchool = new Vector3(-927.5778, -2039.3774, 8.282331);
        ...
        private static List<Vector3> drivingCoords = new List<Vector3>()
        {
            ...
            new Vector3(-704.6472, -1247.343, 10.67442),     //as72
        };
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Сдача прав"), new Vector3(enterSchool.X, enterSchool.Y, enterSchool.Z + 1), 5f, 0.3f, 0, new Color(255, 255, 255));
                var blip = NAPI.Blip.CreateBlip(enterSchool, 0);
                blip.ShortRange = true;
                blip.Name = Main.StringToU16("Лицензирование");
                blip.Sprite = 498;
                blip.Color = 29;
        ...
        private void timer_exitVehicle(Player player)
        {
            NAPI.Task.Run(() =>
            {
                try
                {
                    ...
                    NAPI.Entity.SetEntityPosition(player, new Vector3(-927.5778, -2039.3774, 8.282331));
                    NAPI.Entity.SetEntityDimension(player, 0);
                    Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"Вы провалили экзaмен", 3000);
                }
        ...
        public static void startDrivingCourse(Player player, int index)
        {
            ...
            switch (index)
            {
                case 0:
                    ...
                    var vehicle = NAPI.Vehicle.CreateVehicle(VehicleHash.Sultan, startCourseCoord[0], startCourseRot[0], 30, 30);
                    ...
                    Trigger.PlayerEvent(player, "createCheckpoint", 12, 1, drivingCoords[0] - new Vector3(0, 0, 2), 4, 0, 255, 0, 0);
                    Trigger.PlayerEvent(player, "createWaypoint", drivingCoords[0].X, drivingCoords[0].Y);
        ...
        private void onPlayerEnterDrive(ColShape shape, Player player)
        {
            try
            {
                ...
                if (check + 2 < drivingCoords.Count)
                    Trigger.PlayerEvent(player, "createCheckpoint", 12, 1, drivingCoords[check + 1] - new Vector3(0, 0, 2), 4, 0, 255, 0, 0, drivingCoords[check + 2] - new Vector3(0, 0, 1.12));
                else
                    Trigger.PlayerEvent(player, "createCheckpoint", 12, 1, drivingCoords[check + 1] - new Vector3(0, 0, 2), 4, 0, 255, 0, 0);
                Trigger.PlayerEvent(player, "createWaypoint", drivingCoords[check + 1].X, drivingCoords[check + 1].Y);
            }
            catch (Exception e)
            {
        ...
        public static void OpenDriveSchoolMenu(Player player)
        {
            ...
            menuItem.Text = $"Мотоциклы - {LicPrices[0]}$";
            menu.Add(menuItem);

            menuItem = new Menu.Item("lic_1", Menu.MenuItem.Button);
            menuItem.Text = $"Легковые машины - {LicPrices[1]}$";
            menu.Add(menuItem);

            menuItem = new Menu.Item("lic_2", Menu.MenuItem.Button);
            menuItem.Text = $"Грузовые машины - {LicPrices[2]}$";
            menu.Add(menuItem);
            ...
            menuItem.Text = $"Вертолёты - {LicPrices[4]}$";
            menu.Add(menuItem);

            menuItem = new Menu.Item("lic_5", Menu.MenuItem.Button);
            menuItem.Text = $"Самолёты - {LicPrices[5]}$";
            menu.Add(menuItem);
