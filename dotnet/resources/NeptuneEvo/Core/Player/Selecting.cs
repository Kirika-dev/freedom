...
namespace NeptuneEVO.Core
{
    class Selecting : Script
    {
        ...
        [RemoteEvent("server::animation:stop")]
        public static void animstop(Player player)
        {
            if (player.HasData("PLAYER_PLAY_ANIM") && player.GetData<bool>("PLAYER_PLAY_ANIM") == true) return;
        ...
        {
            if (player.HasData("PLAYER_PLAY_ANIM") && player.GetData<bool>("PLAYER_PLAY_ANIM") == true) return;
            if (ad == "random@arrests@busted" && an == "idle_c" || ad == "random@mugging3" && an == "handsup_standing_base") player.SetData("HANDS_UP", true);
            player.SetSharedData("animData", true);
            player.SetData("animData", true);
            player.PlayAnimation(ad, an, af);
        }
        [RemoteEvent("server::walkstyle:set")]
        public static void setWalkStyle(Player player, string walk)
        {
            player.SetSharedData("playerws", walk);
            NAPI.ClientEvent.TriggerClientEventInRange(player.Position, 250, "Player_SetWalkStyle", player, walk);
            return;
        }
                    ...
                    case "DROPPED":
                        {
                            ...
                            {
                                entity.SetSharedData("PICKEDT", false);
                                nInventory.Add(player, new nItem(item.Type, item.Count - tryAdd, item.Data, wear: item.Wear, subdata: item.subData));
                            ...
                            NAPI.Task.Run(() => { if (player != null) { InvInterface.sendItems(player); } }, 100);
                            Main.PlayAnimation(player, "random@domestic", "pickup_low", 39, 1700);
                            return;
                        }
                    case "WeaponSafe":
                    case "SubjectSafe":
                    case "ClothesSafe":
                        {
                            ...
                            InvInterface.OpenOut(player, items, furniture.Name, FurnitureManager.SafesType[furniture.Name]);
                            return;
                        }
                    case "MoneyBag":
                        {
                            ...
                            NAPI.Task.Run(() => { try { player.PlayAnimation("rcmcollect_paperleadinout@", "kneeling_arrest_get_up", 33); Main.OffAntiAnim(player); } catch { } }, 1700);
                            return;
                        }
                    case "DrillBag":
                        {
        ...
        public static void RM_getvehmanage(Player player)
        {
            try
            {
                ...
                Trigger.PlayerEvent(player, "client::setvehmanage", Utilis.VehiclesName.GetRealVehicleName(VehicleManager.Vehicles[veh.NumberPlate].Model));
                ...
                switch (index)
                {
                    ...
                    case 1:
                        ...
                        if (VehicleStreaming.GetDoorState(vehicle, DoorID.DoorTrunk) == DoorState.DoorOpen)
                        {
                            ...
                            vehicle.SetData("CLOS", vehicle);
                            foreach (var p in NAPI.Pools.GetAllPlayers())
                            {
                                if (p == null || !Main.Players.ContainsKey(p)) continue;
                                if (p.HasData("OPENOUT_TYPE") && p.GetData<int>("OPENOUT_TYPE") == 2 && p.HasData("SELECTEDVEH") && p.GetData<Vehicle>("SELECTEDVEH") == vehicle) InvInterface.Close(p);
                            }
                        }
                        else
                        {
                    ...
                    case 3:
                        ...
                        InvInterface.OpenOut(player, vehicle.GetData<List<nItem>>("ITEMS"), "Багажник", 2);
                    ...
                    case 4:
                        ...
                        BattlePass.AddProgressToQuest(player, 18, 1);
                        return;
                    case 5:
                        ...
                        if (player.HasData("LastActiveWeap"))
                        {
                            ...
                            Commands.RPChat("me", player, $"убрал(а) {nInventory.InventoryItems.Find(x => x.ID == (int)player.GetData<int>("LastActiveWeap")).Name}");
                        }
                        Main.OnAntiAnim(player);
                        ...
                        Trigger.PlayerEventInRange(player.Position, 250, "vehicleattach", player, vehicle);
                    ...
                    case 6:
                        ...
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Заглушите двигатель", 3000);
                            return;
                        }
                        if (vehicle.HasData("PUSHCAR_STATE") && vehicle.GetData<bool>("PUSHCAR_STATE") == true)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Машину уже кто-то толкает", 3000);
                            return;
                        }
                        Trigger.PlayerEvent(player, "client::veh:pushcarstate", true);
                        player.SetData<Vehicle>("VEHICLE_PUSHCAR", vehicle);
                        vehicle.SetData<bool>("PUSHCAR_STATE", true);
                        return;
                    case 7:
                        House house = null;
                        ...
                        var access2 = VehicleManager.canAccessByNumber(player, vehicle.NumberPlate);
                        if (!access2)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У Вас нет ключей от этого транспорта", 3000);
                            return;
                        }
                        setvehPos(player, vehicle);
                        if (house != null)
                        {
                            if (vehicle.Position.DistanceTo(GarageManager.Garages[house.GarageID].Position) < 10)
                            {
                                var garage = GarageManager.Garages[house.GarageID];
                                var veh = garage.GetOutsideCar(vehicle.NumberPlate);
                                if (veh == null) return;
                                VehicleManager.Vehicles[vehicle.NumberPlate].Fuel = (!NAPI.Data.HasEntitySharedData(veh, "PETROL")) ? VehicleManager.VehicleTank[veh.Class] : veh.GetSharedData<int>("PETROL");
                                garage.SendVehicleIntoGarage(vehicle.NumberPlate);
                        ...
                        BattlePass.AddProgressToQuest(player, 7, 1);
                        return;
                }
            }
        ...
        public static void setvehPos(Player player, Vehicle vehicle)
        {
            try
            {
                ...
                {
                    CommandText = "UPDATE `vehicles` SET holder=@hold, model=@model, health=@hp, fuel=@fuel, components=@comp, items=@it,position=@pos,rotation=@rot,keynum=@keyn,dirt=@dirt,sell=@sell,plastic=@plastic,onplast=@onplast WHERE number=@numb"
                };
        ...
        [RemoteEvent("server::veh:exitpushcar")]
        public static void PlayerEvent_ExitPushCar(Player player)
        {
            if (!player.HasData("VEHICLE_PUSHCAR")) return;
            player.GetData<Vehicle>("VEHICLE_PUSHCAR").SetData<bool>("PUSHCAR_STATE", false);
        }
        ...
        public static void PlayerEvent_Trunk(Player player, bool water)
        {
            ...
            if (player.GetData<Vehicle>("VEH").HasData("CLOS"))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Багажник закрыт!", 3000);
                return;
            }
                ...
                switch (action)
                {
                    ...
                    case "Обыскать":
                        if (player.IsInVehicle) return;
                        {
                            ...
                            {
                                if (Main.Players[player].FractionID == 7 || Main.Players[player].FractionID == 9 || Main.Players[player].FractionID == 6 || Main.Players[player].FractionID == 14)
                                {
                            ...
                            foreach (var i in items)
                            {
                                ...
                                if (nInventory.WeaponsItems.Contains(i.Type))
                                    weapons.Add($"{nInventory.InventoryItems.Find(x => x.ID == (int)i.Type).Name} {i.Data}");
                                else
                                    itemNames.Add($"{nInventory.InventoryItems.Find(x => x.ID == (int)i.Type).Name} x{i.Count}");
                            }
                    ...
                    case "Посмотреть паспорт":
                        if (player.IsInVehicle) return;
                        {
                            if (nInventory.Find(Main.Players[target].UUID, ItemType.IDCard) == null)
                            {
                                Notify.Error(player, $"У Гражданина ({target.Value}) нет ID-Карты");
                                return;
                            }
                            if (!target.GetData<bool>("CUFFED"))
                            {
        ...
        public static void playerTransferMoney(Player player, string arg)
        {
            ...
            if (timerold != null)
                timerold.Kill();
            NAPI.Player.PlayPlayerAnimation(player, 1, "mp_common", "givetake2_a");
            TimerEx timer = null;
        ...
        public static void playerHealTarget(Player player, Player target)
        {
            try
            {
                ...
                {
                    var item11 = nInventory.Find(Main.Players[player].UUID, ItemType.Aptechka);
                    if (item11 == null || item11.Count < 1)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"У Вас нет аптечки", 3000);
                        return;
                    }
                    nInventory.Remove(player, ItemType.Aptechka, 1);
                }
        ...
        public static void playerOfferChangeItems(Player player)
        {
            ...
            InvInterface.OpenOut(player, new List<nItem>(), offerMaker.Name, 5);
            InvInterface.OpenOut(offerMaker, new List<nItem>(), player.Name, 5);
