...
namespace NeptuneEVO.Core
{
    class Commands : Script
    {
        ...
        [Command("ts")]
        public static void TESTSOUND(Player player, string a, string b)
        {
            Trigger.PlayerEvent(player, "testsound", a, b);
        }
        ...
        [Command("deldrug")]
        public static void CMD_deldrug(Player player, int id)
        {
            try
            {
                if (Main.Players[player].AdminLVL > 0)
                {
                    ...
                    DrugAddiction.SetDrug(Main.GetPlayerByID(id), 0);
                    return;
                }
                if (Main.Players[player].FractionID != 8) return;
                if (Main.Players[player].FractionLVL < 2)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Для лечения наркозависимости требуется 2 ранг и выше.", 3000);
                    return;
                }
                if (Main.GetPlayerByID(id) == null)
                {
        ...
        [Command("setseat")]
        public static void CMD_setseat(Player player, int id)
        {
            try
            {
                //if (!Main.Players.ContainsKey(player)) return;
                //if (Main.Players[player].AdminLVL > 7)
                Vehicle hVehicle = NAPI.Player.GetPlayerVehicle(player);
                NAPI.Player.WarpPlayerOutOfVehicle(player);
                NAPI.Task.Run(() =>
                {
                    NAPI.Player.SetPlayerIntoVehicle(player, hVehicle, id);
                }, 1000);
                Console.WriteLine(NAPI.Player.GetPlayerVehicleSeat(player));
            }
            catch { }
        }
        ...
        [Command("playerinfo")]
        public static void CMD_playerinfo(Player player, int статический_id)
        {
            try
            {
                if (!Main.Players.ContainsKey(player)) return;
                if (Main.Players[player].AdminLVL > 0)
                {
                    var result = MySQL.QueryRead($"SELECT * FROM `characters` WHERE `idkey`='{статический_id}'");
                    ...
                    foreach(DataRow Row in result.Rows)
                    {
                        ...
                        var name = Convert.ToString(Row["firstname"]);
                        var lastname = Convert.ToString(Row["lastname"]);
                        var lvl = Convert.ToInt32(Row["lvl"]);
                        ...
                        if (target != null)
                        {
                            player.SendChatMessage($"Онлайн: " + $"!{{#13ed4d}}В сети.");
                            player.SendChatMessage($"ID: {target.Value}");
                        }
                        else
                        {
                            player.SendChatMessage($"Онлайн: "+ $"!{{#ed1313}}Не в сети.");
                        }
                        player.SendChatMessage($"Паспорт: {uuid}");
                        player.SendChatMessage($"Имя: {name}");
                        player.SendChatMessage($"Фамилия: {lastname}");
                        player.SendChatMessage($"Уровень: {lvl}");
                        if(fraction == 0)
                        {
                            player.SendChatMessage($"Фракция: Нет");
                        }
                        else
                        {
                            player.SendChatMessage($"Фракция: {Fractions.Manager.FractionNames[fraction]}");
                            player.SendChatMessage($"Лвл во фракции: {fractionlvl}/{Fractions.Configs.FractionRanks[fraction].Count}");
                        }
                    }
                    return;
                }
        ...
        public static void CMD_fakeuid(Player player, int id, string firstname, string lastname)
        {
            try
            {
                if (Main.Players[player].FractionID != 11) return; // сюда айди фракции
            ...
            try
            {
                ...
                if (type == 0) // fractionvehicles
                {
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        var modelName =Row["model"].ToString();
                ...
                else // othervehicles
                {
                    ...
                    cmd.CommandText = "SELECT * FROM `othervehicles` WHERE `type`=@type";
                    cmd.Parameters.AddWithValue("@type", number);
        ...
        public static void CMD_newVehicleNumber(Player player, string oldNum, string newNum)
        {
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "UPDATE vehicles SET number=@number1 WHERE number=@number2";
                cmd.Parameters.AddWithValue("@number1", newNum);
                cmd.Parameters.AddWithValue("@number2", oldNum);
                MySQL.Query(cmd); 
        ...
        public static void CMD_newVehicleMafiaNumber(Player player)
        {
            try
            {
               ...
               var players = Main.GetPlayersInRadiusOfPosition(player.Position, 10);
                ...
                {
                    MySqlCommand cmd = new MySqlCommand();
                    cmd.CommandText = "SELECT * FROM vehicles WHERE holder=@holder";
                    cmd.Parameters.AddWithValue("@holder", player.Name);
                    DataTable res = MySQL.QueryRead(cmd);
        ...
        public static void CMD_offunwarn(Player player, string target)
        {
            try
            {
                ...
                cmd.CommandText = "UPDATE characters SET warns=@warns WHERE firstname=@firstname AND lastname=@lastname";
                cmd.Parameters.AddWithValue("@warns", warns);
                cmd.Parameters.AddWithValue("@firstname", split[0]);
                cmd.Parameters.AddWithValue("@lastname", split[1]);
                MySQL.Query(cmd);
        ...
        [Command("fixcar")] // Починить авто (2 лвл)
        public static void CMD_fixcar(Player player, ushort vid = 0)
        {
            try
            {
        ...
        public static void CMD_showPlayerStats(Player admin, int id)
        {
            try
            {
                ...
                Trigger.PlayerEvent(admin, "client::inventory:setinfo", 2, json);
                admin.SetData("CHANGE_WITH", player);
                InvInterface.OpenOut(admin, nInventory.Items[acc.UUID], player.Name, 20);
            }
            catch (Exception e)
            {
        ...
        public static void ACMD_setfracveh(Player player, string vehname, int rank, int c1, int c2)
        {
            try
            {
                ...
                if (vehicle.HasData("ACCESS") && vehicle.GetData<string>("ACCESS") == "FRACTION")
                {
                    ...
                    var canmeds = (vh == (VehicleHash)NAPI.Util.GetHashKey("emsnspeedo"));
        ...
        [Command("tocar")]
        public static void CMD_tocar(Player player, int id)
        {
            //if (!Group.CanUseCmd(player, "sp")) return;
            try
            {
                player.SetIntoVehicle(NAPI.Pools.GetAllVehicles().Find(x => x.Value == id), 0);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        [Command("metp")]
        ...
        [Command("offdeladmin")]
        public static void CMD_delAdmin(Player player, string имя_фамилия)
        {
            try
            {
                if (!Group.CanUseCmd(player, "deladmin")) return;
                var name = имя_фамилия.Split('_');
                MySQL.Query($"UPDATE `characters` SET `adminlvl`='{0}' WHERE `firstname`='{name[0]}' AND `lastname`='{name[1]}'");
                Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Игрок снят с поста администраци.", 3000);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        [Command("del")]
        public static void CMD_deleteCar(Player player, ushort vid)
        {
            try
            {
                if (!Group.CanUseCmd(player, "veh")) return;
                foreach (var v in NAPI.Pools.GetAllVehicles())
                {
                    if (vid == v.Value)
                    {
                            ...
                            List<Player> occupants = VehicleManager.GetVehicleOccupants(v);
                            if (occupants.Count > 0)
                            {
                                ...
                                v.SetData("OCCUPANTS", newOccupants);
                            }
                            if (VehicleManager.GetVehicleOccupants(v).Count >= 1) continue;
                            if (v.GetData<string>("ACCESS") == "PERSONAL")
                            {
                                Player owner = v.GetData<Player>("OWNER");
                                NAPI.Entity.DeleteEntity(v);                           
                            }
                            else if (v.GetData<string>("ACCESS") == "WORK")
                                Admin.RespawnWorkCar(v);
                            else if (v.GetData<string>("ACCESS") == "FRACTION")
                                Admin.RespawnFractionCar(v);
                            else if (v.GetData<string>("ACCESS") == "GANGDELIVERY" || v.GetData<string>("ACCESS") == "MAFIADELIVERY")
                                NAPI.Entity.DeleteEntity(v);
        ...
        public static void CMD_revive(Player client, int id)
        {
            try
            {
                if (!Group.CanUseCmd(client, "veh")) return;
                Player target = Main.GetPlayerByID(id);
                if (target == null)
                {
                ...
                Trigger.PlayerEvent(target, "screenFadeIn", 0);
        ...
        [Command("veh")]
        public static void CMD_createVehicle(Player player, string name = "comet7", int a = 0, int b = 0, string number = "FREEDOM")
        {
            try
            {
                if (player == null || !Main.Players.ContainsKey(player)) return;
        ...
        [Command("fireshow")]
        public static void CMD_fireshow(Player player, int trys)
        {
            Main.PlayerEventToAll("client::fireshow:play", player.Position.X, player.Position.Y, player.Position.Z, trys);
        }

        [Command("intro")]
        public static void CMD_TestIntroCamera(Player player)
        {
            Trigger.PlayerEvent(player, "client::intro:start");
        }    
        ...
        [Command("sq")]
        public static void CMD_STARTQUEST(Player player)
        {
            Trigger.PlayerEvent(player, "client::questPlane");
        }
        [Command("vehcs")]
        ...
        [Command("flipcar")]
        public static void CMD_flipid(Player player, ushort vid)
        {
            try
            {
                if (!Group.CanUseCmd(player, "flipcar")) return;
                foreach (var v in NAPI.Pools.GetAllVehicles())
                {
                    if (vid == v.Value)
                    {
                        if (!Group.CanUseCmd(player, "fixcar")) return;
                        VehicleManager.RepairCar(v);
                        NAPI.Entity.SetEntityPosition(v, v.Position + new Vector3(0, 0, 2.5f));
                        NAPI.Entity.SetEntityRotation(v, new Vector3(0, 0, v.Rotation.Z));
                        GameLog.Admin($"{player.Name}", $"flipcar", $"");
                    }
                }
            }
            catch (Exception e)
        ...
        [Command("offudemorgan")]
        public static void CMD_offreleaseTargetFromDemorgan(Player player, string имя_фамилия)
        {
            try
            {
                if (!Group.CanUseCmd(player, "udemorgan")) return;
                var name = имя_фамилия.Split('_');
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "UPDATE `characters` SET `demorgan`=0 WHERE `firstname`=@firstname AND `lastname`=@lastname";
                cmd.Parameters.AddWithValue("@firstname", name[0]);
                cmd.Parameters.AddWithValue("@lastname", name[1]);
                MySQL.Query(cmd);
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы выпустили игрока.", 3000);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        public static void CMD_offlineJailTarget(Player player, string target, int time, string reason)
        {
            try
            {
                ...
                var split = target.Split('_');
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "UPDATE `characters` SET `demorgan`=@demorgan,`arrest`=0 WHERE firstname=@firstname AND lastname=@lastname";
        ...
        { 
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = $"update `accounts` set `redbucks`=`redbucks`+{amount} where `login`=@login";
        ...
        [Command("db", GreedyArg = true)]
        public static void CMD_govoocFracChat(Player player, string msg)
        {
            try
            {
                Fractions.Manager.govFractionChat(player, "(( " +  msg + " ))");
        ...
        public static void CMD_gov(Player player, string msg)
        {
            try
            {
                ...
                NAPI.Chat.SendChatMessageToAll($"~b~[{Fractions.Manager.GovTags[frac]}] {split[0]} {split[1]}: ~w~{msg}");
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
                ...
                switch (result)
                {
                    case 3:
                        foreach (Player player in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, sender.Dimension))
                            Trigger.PlayerEvent(player, "sendRPMessage", "try", "!{#ffffff}{name}: " + "!{#9ce25c} Попытался " + "!{#ffffff}" + message + "!{#9ce25c}" + " Удачно", new int[] { sender.Value });
                        return;
                    default:
                        foreach (Player player in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, sender.Dimension))
                            Trigger.PlayerEvent(player, "sendRPMessage", "try", "!{#ffffff}{name}: " + "!{#f31c1c} Попытался " + "!{#ffffff}" + message + "!{#f31c1c}" + " Неудачно", new int[] { sender.Value });
                        return;
                }
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
                ...
                switch (cmd)
                {
                    case "me":
                        foreach (var player in Main.GetPlayersInRadiusOfPosition(sender.Position, 10f, sender.Dimension))
                            Trigger.PlayerEvent(player, "sendRPMessage", "me", "!{#f3a41c}{name} " + message, names);
                        return;
                    case "todo":
