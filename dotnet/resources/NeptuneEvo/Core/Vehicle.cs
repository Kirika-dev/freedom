...
namespace NeptuneEVO.Core
{
    class VehicleManager : Script
    {
        ...
        public static List<VehicleHash> PremiumPetrolVehicles = new List<VehicleHash>()
        {
              (VehicleHash)NAPI.Util.GetHashKey("t20"),
        };
        ...
        public static string Create(string Holder, string Model, Color Color1, Color Color2, Color Color3, int Health = 1000, int Fuel = 100, int Price = 0)
        {
            ...
            data.OnPlast = false;
            string nullText = "        ";
            string Number = nullText + "TRANSIT-" + GenerateNumber();
           ...
           MySQL.Query("INSERT INTO `vehicles`(`number`, `holder`, `model`, `health`, `fuel`, `price`, `components`, `items`,`sell`,`plastic`,`onplast`)" +
            ...
            catch (Exception e) { Log.Write("on create: " + e.ToString()); return "ERROR"; }
        }
        ...
        public static void Remove(string Number, Player player = null)
        {
            ...
            cmd.CommandText = "DELETE FROM `vehicles` WHERE number=@number";
        ...
        public static bool Save(string Number)
        {
            ...
            MySqlCommand cmd = new MySqlCommand();
            cmd.CommandText = "UPDATE `vehicles` SET holder=@hold, model=@model, health=@hp, fuel=@fuel, components=@comp, items=@it,keynum=@keyn,dirt=@dirt,sell=@sell,plastic=@plastic,onplast=@onplast WHERE number=@numb";
        ...
        public static void OpenSellCarMenu(Player player)
        {
            ...
            foreach (string number in getAllPlayerVehicles(player.Name))
            {
                ...
                menuItem.Text = Utilis.VehiclesName.GetRealVehicleName(Vehicles[number].Model) + " - " + number;
                menu.Add(menuItem);
            }
            ...
            {
                menuItem = new Menu.Item(number, Menu.MenuItem.Button);
                menuItem.Text = Utilis.VehiclesName.GetRealVehicleName(AirVehicles.Airs[number].Model) + " - " + number;
                menu.Add(menuItem);
            }
                ...
                case "engineCarPressed":
                    ...
                    switch (NAPI.Data.GetEntityData(vehicle, "ACCESS"))
                    {
                        case "HOTEL":
                            ...
                            {
                                Core.VehicleStreaming.SetEngineState(vehicle, false);
                                Trigger.PlayerEvent(sender, "setRpmNull");
        ...
        public static void changeOwner(string oldName, string newName)
        {
            ...
            lock (AirVehicles.Airs)
            {
                ...
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "UPDATE `airvehicles` SET `holder`=@newName WHERE `holder`=@oldName";
            ...
            lock (Vehicles)
            {
                ...
                // // //
                MySqlCommand cmd = new MySqlCommand();
                cmd.CommandText = "UPDATE `vehicles` SET `holder`=@newName WHERE `holder`=@oldName";
                cmd.Parameters.AddWithValue("@newName", newName);
                cmd.Parameters.AddWithValue("@oldName", oldName);
                MySQL.Query(cmd);
            }
    ...
    class VehicleInventory : Script
    {
        ...
        public static void Add(Vehicle vehicle, nItem item)
        {
            ...
            if (nInventory.ClothesItems.Contains(item.Type) || nInventory.WeaponsItems.Contains(item.Type)
                || nInventory.MeleeWeaponsItems.Contains(item.Type) || item.Type == ItemType.CarKey || item.Type == ItemType.KeyRing || nInventory.IgnoreItems.Contains(item.Type))
            {
                items.Add(item);
            }
            else
            {
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    if (items[i].Type == item.Type && items[i].Count < nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks)
                    {
                        int temp = nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks - items[i].Count;
                ...
                while (count > 0)
                {
                    if (count >= nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks)
                    {
                        items.Add(new nItem(item.Type, nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks, item.Data));
                        count -= nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks;
                    }
                    else
                    {
            ...
            foreach (Player p in Main.Players.Keys.ToList())
            {
                ...
                if (p.HasData("OPENOUT_TYPE") && p.GetData<int>("OPENOUT_TYPE") == 2 && p.HasData("SELECTEDVEH") && p.GetData<Vehicle>("SELECTEDVEH") == vehicle) InvInterface.OpenOut(p, vehicle.GetData<List<nItem>>("ITEMS"), "Багажник", 2);
            }
        }
        ...
        public static int TryAdd(Vehicle vehicle, nItem item)
        {
            ...
            if (nInventory.ClothesItems.Contains(item.Type) || nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type) || 
                item.Type == ItemType.CarKey || item.Type == ItemType.KeyRing || nInventory.IgnoreItems.Contains(item.Type))
            {
            ...
            else
            {
                ...
                foreach (nItem i in items)
                    if (i.Type == item.Type) count += nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks - i.Count;

                int slots = 60;
                int maxCapacity = (slots - items.Count) * nInventory.InventoryItems.Find(x => x.ID == (int)item.Type).Stacks + count;
        ...
        public static void Remove(Vehicle vehicle, nItem item)
        {
            ...
            if (nInventory.ClothesItems.Contains(item.Type) || nInventory.WeaponsItems.Contains(item.Type) || nInventory.MeleeWeaponsItems.Contains(item.Type) || 
                item.Type == ItemType.BagWithDrill || item.Type == ItemType.BagWithMoney || item.Type == ItemType.CarKey || item.Type == ItemType.KeyRing || nInventory.IgnoreItems.Contains(item.Type))
            {
                items.Remove(item);
            }
            else
