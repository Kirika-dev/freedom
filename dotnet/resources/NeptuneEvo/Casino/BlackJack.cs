...
namespace NeptuneEVO.Casino
{
    class BlackJack : Script
    {
        ...
        public static void BetDown(Player player)
        {
            ...
            Trigger.PlayerEvent(player, "casinoKeys", "setBet", chips - BlackJackTableMinBet[player.GetData<int>("BJ_TABLE")], BlackJackTableMinBet[player.GetData<int>("BJ_TABLE")]);
        }
        ...
        public static void Bet(Player player)
        {
            ...
            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы сделали ставку в размере {chips} фишек", 3000);
            Trigger.PlayerEventInRange(player.Position, 20, "bet_blackjack", player.GetData<int>("BJ_TABLE"), player.GetData<int>("BJ_SEAT"), player.Handle);
            player.SetData<bool>("BlackJackMakeBet", true);
        ...
        public static void Seat(Player player, int table, int seat)
        {
            ...
            Trigger.PlayerEventInRange(GET_TABLE_CORD(table), 20, "seat_to_blackjack_table", table, seat, player.Handle);
            Trigger.PlayerEvent(player, "casinoKeys", "show");
            Trigger.PlayerEvent(player, "casinoKeys", "setBet", BlackJackTableMinBet[table], BlackJackTableMinBet[table]);
            //Trigger.PlayerEvent(player, "seat_to_blackjack_table", 1, 0);
        }

        public static void SB(Player player)
        {
            Trigger.PlayerEvent(player, "openBlackjack");
        }
        ...
        public static void Exit(Player player)
        {
            ...
            if (player.GetData<int>("BJ_STATUS") != 0)
                return;
            if (player.GetData<bool>("BlackJackMakeBet"))
            {
                return;
            }
            if (BlackJackPlayers[player.GetData<int>("BJ_TABLE")].Contains(player))
            ...
            Trigger.PlayerEvent(player, "casinoKeys", "hide");
            Trigger.PlayerEventInRange(GET_TABLE_CORD(player.GetData<int>("BJ_TABLE")), 20, "exit_table", player.Handle);
            ExitPlayer(player);
            //Trigger.PlayerEvent(player, "seat_to_blackjack_table", 1, 0);
        }
                ...
                while (true)
                {
                    if (BlackJackBetTime[tb] != 0)
                    {
                        if (BlackJackSeatsPlayers[tb].Count != 0)
                        {
                            ...
                            {
                                //Trigger.PlayerEvent(BlackJackSeatsPlayers[tb][i], "blackjack_show_bet", 0, 0);
                               // BlackJackSeatsPlayers[tb][i].SendChatMessage($"Ожидание ставок [ {BlackJackBetTime[tb]} ]");
                                Trigger.PlayerEvent(BlackJackSeatsPlayers[tb][i], "casinoKeys", "setTime", 0, 0);
                            }
                    ...
                    for (int i = 0; i < BlackJackSeatsPlayers[tb].Count; i++)
                    {
                        Trigger.PlayerEvent(BlackJackSeatsPlayers[tb][i], "casinoKeys", "toggleStart", true);
                    }
                    ...
                    for (int i = 0; i < BlackJackPlayers[tb].Count; i++)
                    {
                        ...
                        Thread.Sleep(2000);
                        Trigger.PlayerEvent(BlackJackPlayers[tb][i], "casinoKeys", "setChips", BlackJackPlayers[tb][i].GetData<int>("BJ_SUM").ToString(), BlackJackDealSum[tb].ToString());
                    }
                        ...
                        while (true)
                        {
                            try
                            {
                                if (BlackJackPlayers[tb][i].GetData<int>("BJ_ANSWER") == 0)
                                {
                                    ...
                                    if (res <= 0)
                                    {
                                        Trigger.PlayerEvent(BlackJackPlayers[tb][i], "casinoKeys", "toggleStart", false);
                                        ...
                                        Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "lose_blackjack", tb);
                                        Thread.Sleep(1500);
                                    ...
                                    else if (res == 1)
                                    {
                                        ...
                                        BlackJackPlayers[tb][i].SetData("BJ_ANSWER", 0);
                                        Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "win_blackjack", tb);
                                        nInventory.Add(BlackJackPlayers[tb][i], new nItem(ItemType.CasinoChips, BlackJackPlayers[tb][i].GetData<int>("BJ_BET") * 2));
                                        Thread.Sleep(1500);
                                        CleanPlayer(BlackJackPlayers[tb][i]);
                                        break;
                                    }
                                    Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "stand_or_hit", tb, BlackJackPlayers[tb][i].GetData<int>("BJ_SEAT"));
                                   // Trigger.PlayerEvent(BlackJackPlayers[tb][i], "blackjack_show_game");
                                ...
                                else if (BlackJackPlayers[tb][i].GetData<int>("BJ_ANSWER") == 1)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Trigger.PlayerEvent(BlackJackPlayers[tb][i], "casinoKeys", "setTime", 0,  15 - BlackJackPlayers[tb][i].GetData<int>("BJ_TIMER") / 1000);
                                        //Trigger.PlayerEvent(BlackJackSeatsPlayers[tb][i], "blackjack_update_time", $"{15 - BlackJackPlayers[tb][i].GetData<int>("BJ_TIMER") / 1000}");
                                    }
                                }
                                else
                                {
                                    ...
                                    {
                                        //Trigger.PlayerEvent(BlackJackPlayers[tb][i], "blackjack_hide_game");
                                        GetCard(BlackJackPlayers[tb][i]);
                                        BlackJackPlayers[tb][i].SetData("BJ_ANSWER", 0);
                                        BlackJackPlayers[tb][i].SetData("BJ_TIMER", 0);
                                        
                                        //Trigger.PlayerEvent(BlackJackPlayers[tb][i], "blackjack_update_result", BlackJackDealSum[tb], BlackJackPlayers[tb][i].GetData<int>("SUM"));
                                    }
                                    else
                                    {
                                        BlackJackPlayers[tb][i].SetData("BJ_ANSWER", 0);
                                    ...
                                    Trigger.PlayerEvent(BlackJackPlayers[tb][i], "casinoKeys", "setChips", BlackJackPlayers[tb][i].GetData<int>("BJ_SUM"), BlackJackDealSum[tb]);
                                }
                            }
                            catch (Exception e)
                            {
                                Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "clean_cards", tb, BlackJackTableSeatsNow[tb]);
                                Thread.Sleep(2000);
                                Log.Debug(e.Message);
                                break;
                            }
                    ...
                    {
                        Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "flip_card", tb);
                    ...
                    {
                        Trigger.PlayerEvent(BlackJackPlayers[tb][i], "casinoKeys", "setChips", BlackJackPlayers[tb][i].GetData<int>("BJ_SUM"), BlackJackDealSum[tb]);
                        Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "clean_cards", tb, BlackJackPlayers[tb][i].GetData<int>("BJ_SEAT"));
                        //Trigger.PlayerEvent(BlackJackPlayers[tb][i], "blackjack_hide_game");
                    ...
                    Trigger.PlayerEventInRange(GET_TABLE_CORD(tb), 20, "clean_cards", tb, 4);
        ...
        public static void CleanPlayer(Player player)
        {
            ...
            player.SetData<bool>("BlackJackMakeBet", false);
        ...
        public static void ExitPlayer(Player player)
        {
            ...
            Trigger.PlayerEvent(player, "blackjack_hide");
        }
        ...
        public static void CheckFinalSum(Player player)
        {
            ...
            if (mainSum > 21 && pSum > 21)
            {
                ...
                Trigger.PlayerEventInRange(GET_TABLE_CORD(player.GetData<int>("BJ_TABLE")), 20, "lose_blackjack", player.GetData<int>("BJ_TABLE"));
                Thread.Sleep(1500);
            }
            else if (mainSum > 21 && pSum <= 21)
            {
                Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"Вы выиграли {bet * 2} фишек", 3000);
                Trigger.PlayerEventInRange(GET_TABLE_CORD(player.GetData<int>("BJ_TABLE")), 20, "win_blackjack", player.GetData<int>("BJ_TABLE"));
        ...
        public static void GetCard(Player player)
        {
            ...
            Trigger.PlayerEventInRange(GET_TABLE_CORD(table), 20, "client_bj_give_card", table, seat, emptySlot, randomCard);
        }
        ...
        public static void CMD_Hit(Player player)
        {
            ...
            {
                Trigger.PlayerEventInRange(player.Position, 150, "decline_card", player.Handle);
        ...
        public static void CMD_Stand(Player player)
        {
            ...
            if (player.GetData<int>("BJ_ANSWER") == 1)
            {
                Trigger.PlayerEventInRange(player.Position, 150, "request_card", player.Handle);
        ...
        public static void GetCardDeal(int table)
        {
            ...
            Trigger.PlayerEventInRange(GET_TABLE_CORD(table), 20, "client_bj_give_card", table, 4, emptySlot, randomCard);
        }
