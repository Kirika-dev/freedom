    ...
    class FractionCommands : Script
    {
        ...
        public static void InviteToFraction(Player sender, Player target)
        {
            if (Manager.canUseCommand(sender, "invite"))
            {
                ...
                {
                    Notify.Error(sender, $"У Гражданина ({target.Value}) нет ID-Card");
                    return;
                }
                ...
                target.SetSharedData("fchat", true); //todo chat
                if (Manager.FractionTypes[Main.Players[target].FractionID] == 2)
                {
                    target.SetSharedData("dchat", true); //todo chat
                }
            }
        }
        ...
        public static void UnInviteFromFraction(Player sender, Player target, bool mayor = false)
        {
            ...
            target.ResetSharedData("fchat"); //todo chat
            if (Manager.FractionTypes[Main.Players[target].FractionID] == 2)
            {
                target.ResetSharedData("dchat"); //todo chat
            }
        ...
        public static void robberyTarget(Player player, Player target)
        {
            ...
            var max = (Main.Players[target].Money >= 500000) ? 500000 : Convert.ToInt32(Main.Players[target].Money) - 2000;
        ...
        public static void healTarget(Player player, Player target, int price)
        {
            if (Manager.canUseCommand(player, "heal"))
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны установить цену от 60.000$ до 100.000$", 3000);
                    return;
                }
