...
namespace NeptuneEVO.Fractions
{
    class CarDelivery : Script
    {
        ...
        private static Dictionary<int, Vector3> MafiaStartDelivery = new Dictionary<int, Vector3>()
        {
            ...
            { 12, new Vector3(-1551.744, 131.372, 55.66665) },
            { 13, new Vector3(-1919.11, 2056.85, 139.74) },
        };
        ...
        private static Vector3 PoliceEndDelivery = new Vector3(485.2763, -1025.6108, 26.934687);
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel("~b~Джимми", GangStartDelivery + new Vector3(0, 0, 2.5), 5f, 0.4f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                foreach (var pos in MafiaStartDelivery)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel("~b~Взять задание", pos.Value, 5f, 0.4f, 0, new Color(255, 255, 255));
                }
                #endregion
                ...
                colShape.OnEntityEnterColShape += (s, e) =>
                {
                    try
                    {
                        ...
                        e.Vehicle.Delete();
                        MoneySystem.Wallet.Change(e, 2000000);
                        GameLog.Money($"server", $"player({Main.Players[e].UUID})", 2500, $"arrestCar");
                ...
                foreach (var pos in GangEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            NAPI.Data.ResetEntityData(e.Vehicle.GetData<Vehicle>("WHOS_VEH"), "DELIVERY_CAR");
                            e.Vehicle.Delete();
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 1000000;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 2500, "dropCar");
                            e.SendChatMessage("Сдача машины: !{#00FF00}2500$ ~w~были отправлены в общак банды. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()}" + ")");
                        }
                ...
                foreach (var pos in MafiaEndDelivery)
                {
                    ...
                    colShape.OnEntityEnterColShape += (s, e) =>
                    {
                        try
                        {
                            ...
                            e.Vehicle.Delete();
                            Stocks.fracStocks[Main.Players[e].FractionID].Money += 1000000;
                            GameLog.Money($"server", $"frac({Main.Players[e].FractionID})", 2700, "dropCar");
                            e.SendChatMessage("Сдача фургона: !{#00FF00}2700$ ~w~были отправлены в общак мафии. (" + $"{DateTime.Now.ToShortDateString()} {DateTime.Now.ToShortTimeString()})");
                        }
            ...
            try
            {
                ...
                switch ((string)vehicle.GetData<string>("ACCESS"))
                {
                    case "GANGDELIVERY":
                        ...
                        {
                            Trigger.PlayerEvent(player, "createWaypoint", PoliceEndDelivery.X, PoliceEndDelivery.Y);
                        ...
                        else
                        {
                            ...
                            Trigger.PlayerEvent(player, "createWaypoint", GangEndDelivery[end].X, GangEndDelivery[end].Y);
                    ...
                    case "MAFIADELIVERY":
                        ...
                        {
                            var end = (int)vehicle.GetData<int>("ENDPOINT");
                            Trigger.PlayerEvent(player, "createWaypoint", MafiaEndDelivery[end].X, MafiaEndDelivery[end].Y);
                            Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, "Доставьте машину в точку, отмеченную на карте", 3000);
                        }
                        return;
                }
        ...
        public static void Event_InteractPressed(Player player, int id)
        {
            ...
            switch (id)
            {
                case 52:
                    ...
                    Trigger.PlayerEvent(player, "gangmis");
                    return;
                case 53:
                    if (fraction != player.GetData<int>("ONMAFIAID")) return;
                    if (fraction != player.GetData<int>("ONMAFIAID")) return;
                    Trigger.PlayerEvent(player, "mafiamis");
                    return;
            }
        }
            ...
            try
            {
                ...
                switch (id)
                {
                    ...
                    case 1:
                        ...
                        Trigger.PlayerEvent(player, "createWaypoint", GangSpawnAutos[LastGangSpawn].X, GangSpawnAutos[LastGangSpawn].Y);
