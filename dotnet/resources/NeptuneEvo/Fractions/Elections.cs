...
namespace NeptuneEVO.Fractions
{
    class ElectionsSystem : Script
    {
                                ...
                                if (!CheckPlayerVoted(Main.Accounts[player].Login, ElectionPointsList[i].Election))
                                {
                                    ...
                                    for (int l = 0; l != ElectionList.Count; l++)
                                    {
                                        if (ElectionList[l].Election == ElectionPointsList[i].Election)
                                        {
                                            ...
                                            Trigger.PlayerEvent(player, "openelem", ElectionList[l].Name); // Первый итем добавляется вместе с созданием самой меню natifveui, иначе рандомно случается краш, не у всех и не всегда
                                            second = l + 1;
                                            break;
                                        }
                                    }
                                    ...
                                    {
                                        if (ElectionList[l].Election == ElectionPointsList[i].Election) Trigger.PlayerEvent(player, "addcandidate", ElectionList[l].Name); // Добавление всех остальных кандидатов к текущей менюшке
                                    }
                                }
                                else player.SendChatMessage("Вы уже голосовали в этих выборах.");
                            }
        ...
        public void AddPoint_CMD(Player Player, uint ElectionID, bool isOpened = false)
        {
            ...
            {
                Player.SendChatMessage("Такой Election уже существует!");
                Notify.Send(Player, NotifyType.Error, NotifyPosition.BottomCenter, "Такой Election уже существует!", 3000);
                return;
            }
