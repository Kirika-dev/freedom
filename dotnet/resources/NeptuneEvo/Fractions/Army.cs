    ...
    class Army : Script
    {
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                Cols.Add(9, NAPI.ColShape.CreateCylinderColShape(ArmyCheckpoints[9], 5, 6, 0));
                Cols[9].SetData("INTERACT", 5233);
                Cols[9].OnEntityEnterColShape += onEntityEnterColshape;//enter
                Cols[9].OnEntityExitColShape += onEntityExitArmyMats;

                Cols.Add(10, NAPI.ColShape.CreateCylinderColShape(ArmyCheckpoints[10], 5, 6, 0));
                Cols[10].SetData("INTERACT", 5234);//exit
                Cols[10].OnEntityEnterColShape += onEntityEnterColshape;
                Cols[10].OnEntityExitColShape += onEntityExitArmyMats;
                ...
                NAPI.Marker.CreateMarker(1, ArmyCheckpoints[9] - new Vector3(0, 0, 1.2), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));
                NAPI.Marker.CreateMarker(1, ArmyCheckpoints[10] - new Vector3(0, 0, 1.2), new Vector3(), new Vector3(), 1f, new Color(255, 255, 255, 220));
            }
        ...
        {
            new Vector3(-2353.1099, 3255.0464, 91.90369), // guns     0
            new Vector3(-2357.3816, 3256.4343, 32.810706), // dressing room    1
            ...
            new Vector3(-2362.9685, 3246.1284, 91.90369), // army stock    5
            ...
            new Vector3(-2345.4146, 3232.5203, 34.747033), // ENTER   9
            new Vector3(-2343.649, 3266.8804, 32.810775), // EXIT   10
        };
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 60: // open stock gun
                    ...
                    InvInterface.OpenOut(player, Stocks.fracStocks[14].Weapons, "Склад оружия", 6);
                    return;
                case 5233:
                    if (Main.Players[player].FractionID != 14)
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не сотрудник Army", 3000);
                        break;
                    }
                    if (player.IsInVehicle) return;
                    if (player.HasData("FOLLOWING"))
                    ...
                    NAPI.Entity.SetEntityPosition(player, ArmyCheckpoints[10]);
                    return;
                case 5234:
                    if (Main.Players[player].FractionID != 14)
                    {
                    ...
                    NAPI.Entity.SetEntityPosition(player, ArmyCheckpoints[9]);
                    return;
            }
        }
