    ...
    class Ems : Script
    {
        ...
        { 
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Лифт в больницу"), new Vector3(emsCheckpoints[0].X, emsCheckpoints[0].Y, emsCheckpoints[0].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                NAPI.Marker.CreateMarker(1, emsCheckpoints[0], new Vector3(), new Vector3(), 0.5f, new Color(217, 207, 255), false, 0);
                ...
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Лифт на парковку"), new Vector3(emsCheckpoints[1].X, emsCheckpoints[1].Y, emsCheckpoints[1].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                NAPI.Marker.CreateMarker(1, emsCheckpoints[1], new Vector3(), new Vector3(), 0.5f, new Color(217, 207, 255), false, 0);
                ...
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Переодевалка"), new Vector3(emsCheckpoints[4].X, emsCheckpoints[4].Y, emsCheckpoints[4].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[5], 1, 2, 0); // start heal course
                col.SetData("INTERACT", 19);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~"), new Vector3(emsCheckpoints[5].X, emsCheckpoints[5].Y, emsCheckpoints[5].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[6], 1, 2, 0); // tattoo delete
                col.SetData("INTERACT", 51);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Хирург"), new Vector3(emsCheckpoints[6].X, emsCheckpoints[6].Y, emsCheckpoints[6].Z + 0.3), 5F, 0.3F, 0, new Color(255, 255, 255));
                ...
                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[9], 1, 2, 0);
                col.SetData("INTERACT", 2222);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Лифт в больницу"), new Vector3(emsCheckpoints[9].X, emsCheckpoints[9].Y, emsCheckpoints[9].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                NAPI.Marker.CreateMarker(1, emsCheckpoints[9], new Vector3(), new Vector3(), 0.5f, new Color(217, 207, 255), false, 0);

                col = NAPI.ColShape.CreateCylinderColShape(emsCheckpoints[10], 1, 2, 0);
                col.SetData("INTERACT", 2223);
                col.OnEntityEnterColShape += emsShape_onEntityEnterColShape;
                col.OnEntityExitColShape += emsShape_onEntityExitColShape;
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~g~Лифт на крышу"), new Vector3(emsCheckpoints[10].X, emsCheckpoints[10].Y, emsCheckpoints[10].Z + 1), 5F, 0.3F, 0, new Color(255, 255, 255));
                NAPI.Marker.CreateMarker(1, emsCheckpoints[10], new Vector3(), new Vector3(), 0.5f, new Color(217, 207, 255), false, 0);

                #endregion

                for (int i = 3; i < emsCheckpoints.Count; i++)
                {
                    Marker marker = NAPI.Marker.CreateMarker(1, emsCheckpoints[i], new Vector3(), new Vector3(), 0.5f, new Color(217, 207, 255), false, 0);
                }
            }
        ...
        public static List<Vector3> emsCheckpoints = new List<Vector3>()
        {
            ...
            new Vector3(312.11227, -592.51575, 42.164062),  // start heal course        5 ОК
            new Vector3(314.95364, -570.0122, 42.16406), // tattoo delete            6 ОК
            new Vector3(329.47092, -600.88324, -402.1641), // holl                     7 DEL
            new Vector3(340.46616, -580.7759, -207.676838), // to parkovka                  8 DEL
            new Vector3(339.31604, -584.2509, 73.0417), // roof                     9
            new Vector3(327.51837, -603.19037, 42.164055), // лифт на крышу                  10
        };
        ...
        public void onPlayerDeathHandler(Player player, Player entityKiller, uint weapon)
        {
            try
            {
                ...
                var pos = new Vector3(316.15222, -582.3583, 43.364104);
                MySQL.Query($"UPDATE `characters` SET `pos`='{JsonConvert.SerializeObject(pos)}' WHERE `firstname`='{Main.Players[player].FirstName}' AND `lastname`='{Main.Players[player].LastName}'");
                if (player.HasData("ISCAPTURE") && player.GetData<bool>("ISCAPTURE"))
                {
                    player.SetData("ISCAPTURE", false);
                    player.SetData("GANGPOINT", -1);
                    player.SetSharedData("InDeath", false);
                    player.Health = 100;
                    NAPI.Player.SpawnPlayer(player, emsCheckpoints[2]);
                    player.Dimension = 0;
                    return;
                }
                ...
                Jobs.Builder.Event_PlayerDeath(player, entityKiller, weapon);
                Jobs.DeliveryClub.Event_PlayerDeath(player, entityKiller, weapon);

                //Miner=======
                player.SetSharedData("PickAxe.InHands", false);
                if (nInventory.Find(Main.Players[player].UUID, ItemType.PickAxe) != null && nInventory.Find(Main.Players[player].UUID, ItemType.PickAxe).IsActive == true)
                {
                    nInventory.Find(Main.Players[player].UUID, ItemType.PickAxe).IsActive = false;
                }
                //============

                //Sawmill=======
                player.SetSharedData("Axe.InHands", false);
                if (nInventory.Find(Main.Players[player].UUID, ItemType.Axe) != null && nInventory.Find(Main.Players[player].UUID, ItemType.Axe).IsActive == true)
                {
                    nInventory.Find(Main.Players[player].UUID, ItemType.Axe).IsActive = false;
                }
                //============
                ...
                FastSlots.RemoveAllFastSlots(player);

                if (entityKiller != null && !player.HasData("IS_STATE") && entityKiller != player)
                {
                    GameUI.AddToStatistic(player, 3, 1);
                    GameUI.AddToStatistic(entityKiller, 2, 1);
                    Statictic stats = Main.Players[player].Statictica;  
                ...
                if (!player.HasData("IS_DYING"))
                {
                    ...
                    if (entityKiller != null && entityKiller != player && Main.Players.ContainsKey(player))
                        text =  $"Вас убил(a) {Core.Character.Character.GetGender(player)} ({entityKiller.Id}) #{entityKiller.GetSharedData<int>("UID")}";

                    Trigger.PlayerEvent(player, "openDialogMED", text, $"Не время умирать! хотите вызвать медиков ({medics} в сети)?");
                }
                else
                {
                    NAPI.Task.Run(() => {
        ...
        public static void interactPressed(Player player, int interact)
        {
            switch (interact)
            {
                ...
                case 19:
                    ...
                    BattlePass.AddProgressToQuest(player, 20, 1);
                    return;
                case 51:
                ...
                case 58:
                    ...
                    if (!player.IsInVehicle || VehicleManager.Vehicles[player.Vehicle.NumberPlate].Model != "emsnspeedo")
                    {
                ...
                case 2222:
                    ...
                    NAPI.Entity.SetEntityPosition(player, emsCheckpoints[10] + new Vector3(0, 0, 1));
                    return;
                case 2223:
                    if (Main.Players[player].FractionID != 8)
                    {
                    ...
                    NAPI.Entity.SetEntityPosition(player, emsCheckpoints[9] + new Vector3(0, 0, 1));
                    return;
            }
        }
