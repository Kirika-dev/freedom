...
namespace NeptuneEVO.Fractions.Activity
{
                ...
                _ShapeBox.OnEntityExitColShape += (s, e) =>
                {
                    if (e.HasData("isLootingAmmoBox"))
                    {
                        if (e.VehicleSeat == 0)
                        {
                            ...
                            Notify.Alert(e, "Вы выехали из зоны погрузки. Загрузка боеприпасов отменена");
                        }
                    }

                };
            }
            RLog.Write("AmmunationBox Loaded", nLog.Type.Success);
        }
