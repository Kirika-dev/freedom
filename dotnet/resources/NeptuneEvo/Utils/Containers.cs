...
namespace NeptuneEVO.Utils
{
    class ContainerSystem : Script
    {
      ...
      /*  [ServerEvent(Event.ResourceStart)]
        ...
        public static void OpenMenuContainer(Player player)
        {
            ...
            if (container.label.Text == $"Контейнер №{container.ID} \n ~r~Аукцион не проводится") return;
            if (!player.HasData($"ASDsadas_{container.ID}") && !player.HasData($"CONTAINER_PRIZE_{container.ID}"))
            {
                Trigger.PlayerEvent(player, "openContainerMenu", container.ID, container.Type, container.Name, container.Price, container.Price + 10000, container.NamePlayerBet);
                return;
            }
            else if (player.HasData($"ASDsadas_{container.ID}"))
            {
                if (player.GetData<bool>($"ASDsadas_{container.ID}") == false) return;
                Trigger.PlayerEvent(player, "openMenuContainerOpener", container.ID, container.Type, container.Name, container.Price, container.Price, container.NamePlayerBet);
                return;
            }
            else if (player.HasData($"CONTAINER_PRIZE_{container.ID}"))
            {
                if (player.GetData<bool>($"CONTAINER_PRIZE_{container.ID}") == false) return;
                Trigger.PlayerEvent(player, "openPrizMenu", Utilis.VehiclesName.GetRealVehicleName(container.NamePrize), container.SellPricecPrize, container.ID, container.Type, container.Name, container.Price, container.Price + 10000, container.NamePlayerBet);
                return;
            }
        }
        #endregion
        ...
        public static void SetNewBet(Player player, int bet)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Минимальная ставка {container.Price + 10000}.", 3000);
                return;
            }
            else if (bet < container.Price)
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Минимальная ставка {container.Price}.", 3000);
                return;
            }
            if (container.NameBetPlayer2 == player.Name.Replace('_', ' '))
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы уже поставили ставку на этот контейнер.", 3000);
                return;
            }
            container.Price = bet;
            ...
            {
                if (!p.HasData("CONTAINER_PRICE_RETURN")) return;
                if (container.NameBetPlayer2 == p.Name.Replace('_', ' '))
                {
                    MoneySystem.Wallet.Change(p, p.GetData<int>("CONTAINER_PRICE_RETURN"));
                    Notify.Send(p, NotifyType.Info, NotifyPosition.BottomCenter, "Вашу ставку перебили", 3000);
                }
            }
            container.NameBetPlayer2 = player.Name.Replace('_', ' ');
            int timer = 30;
            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы успешно поставили ставку {bet}$", 3000);
        ...
        public static void QueryReadIdCont(Player player)
        {
            ...
            {
                Trigger.PlayerEvent(player, "SetMaxBet", container.Price + 10000);
            }
            else
            {
                Trigger.PlayerEvent(player, "SetMaxBet", container.Price);
            }
        }
        ...
        public static void GetPriz(Player player)
        {
            try
            {
                ...
                if (house == null || house.GarageID == 0)
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Ваш приз - {Utilis.VehiclesName.GetRealVehicleName(vName)}", 2500);
                    return;
                }
                else
                {
                    ...
                    if (vNumber != "none")
                    {
                        ...
                        Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Ваш приз - {Utilis.VehiclesName.GetRealVehicleName(vName)} будет доставлен в гараж", 2500);
                    }
                }
            }
            catch { }
        ...
        public static void OpenContainer(Player player)
        {
            try
            {
                if (!player.HasData("ContainerID")) return;
                Container cont = containers[player.GetData<int>("ContainerID")];
                cont.label.Text = $"Контейнер №{cont.ID} \n ~r~Победил: {player.Name.Replace('_', ' ')}";
    ...
    public class Container
    {
        ...
        public Container(int id, string name, Vector3 pos, Vector3 rot, int price, string type, bool donate = false, string model = "prop_container_02a", string door_l = "prop_cntrdoor_ld_l", string door_r = "prop_cntrdoor_ld_r", string fence = "prop_fncsec_01b")
        {
            ...
            label = NAPI.TextLabel.CreateTextLabel($"Контейнер №{ID} \n ~r~Аукцион не проводится", pos + new Vector3(-2, 6.7, 1), 10f, 0.2f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
        ...
        public void Visible(bool state)
        {
            if (state)
            {
                label.Text = $"Контейнер №{ID} \n ~w~Тип: ~o~{Name} \n ~w~Начальная ставка: ~g~{Price}$";
                marker.Color = new Color(72, 163, 215, 180);
            }
            else
            {
                label.Text = $"Контейнер №{ID} \n ~r~Аукцион не проводится";
        ...
        public void GenerateLoot(Player player, Container cont)
        {
            ...
            string vNumber = VehicleManager.GenerateNumber();
            veh.Dimension = 0;
            ...
            for(int i = 0; i < Businesses.AutoShopI.ProductsList.Count; i++)
            {
                if (!Businesses.AutoShopI.ProductsList[i].ContainsKey(vName)) continue;
                price = Convert.ToInt32(Businesses.AutoShopI.ProductsList[i][vName] / 2);
                break;
            }
