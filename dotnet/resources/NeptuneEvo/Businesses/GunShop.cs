    ...
    class GunShopI : Script
    {
        ...
        public class GunShop : BCore.Bizness
        {
            ...
            static new readonly Dictionary<string, int> Products = new Dictionary<string, int>
            {
				...
				{"DoubleAction", 250000},
                {"Revolver", 300000},
                {"BullpupShotgun", 100000},
                {"CombatPDW", 300000},
                {"MachinePistol", 275000},
				{"Hammer", 15000},
				{"Crowbar", 20000},
				{"GolfClub", 30000},
				{"Hatchet", 80000},
            ...
            public GunShop(int id, string owner, Vector3 position, Vector3 matposition, int cost, int mafia, int bankid, int mat, Vector3 infopoint, int ear) : base(id, owner, position, matposition, cost, mafia, bankid, mat, infopoint, ear)
            {
                Type = 6;
                Name = "Магазин оружия";
                BlipColor = 4;
            ...
            public static void BuyAmmo(Player player, string text1, string text2)
            {
                try
                {
                    ...
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы купили {nInventory.InventoryItems.Find(x => x.ID == (int)AmmoTypes[category]).Name} x{ammo} за {totalPrice}$", 3000);
                }
