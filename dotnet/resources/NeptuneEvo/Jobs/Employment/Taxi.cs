...
namespace NeptuneEVO.Jobs
{
    class Taxi : Script
    {
        ...
        public static Vector3 Position = new Vector3(899.9929, -173.4167, 72.89657);

        public static int taxiRentCost = 50000;
        ...
        public void OnResourceStart()
        {
            try
            {
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Взять такси"), new Vector3(899.9929, -173.4167, 72.89657 + 1.2), 30f, 0.5f, 0, new Color(255, 255, 255));
        ...
        public static void offerTaxiPay(Player player, Player target, int price)
        {
            if (Main.Players[player].WorkID == 3)
            {
                if (NAPI.Data.GetEntityData(player, "WORK") != null)
                {
                    ...
                    {
                        if (price > 70000 || price < 1)
                        {
                            Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не можете установить цену ниже 1$ и выше 70000$", 3000);
                            return;
                        }
