...
namespace NeptuneEVO.Jobs
{
    class Truckers : Script
    {
        ...
        public static Vector3 Position = new Vector3(596.5147, -3046.633, -15.049733);
        ...
        public void OnResourceStart()
        {
            try
            {
                for (int i = 0; i < getProduct.Count; i++)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Доставка"), new Vector3(getProduct[i].X, getProduct[i].Y, getProduct[i].Z + 1), 30f, 0.5f, 4, new Color(255, 255, 255));
                }

                ColShape Shape = NAPI.ColShape.CreateCylinderColShape(Position, 1F, 2F); //596.5147, -3046.633, 5.049733
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Взять грузовик"), new Vector3(596.5147, -3046.633, -15.049733 + 0), 30f, 0.5f, 4, new Color(255, 255, 255)); //596.5147, -3046.633, 5.049733 + 1.2
       ...
       private void timer_playerExitWorkVehicle(Player player, Vehicle vehicle)
        ...
        public static void onEntityEnterDropTrailer(ColShape shape, Player player)
        {
            try
            {
                ...
                player.SetData("PAYMENT", player.GetData<int>("PAYMENT") + (level + goshape * 50));
                Trigger.PlayerEvent(player, "JobStatsInfo", player.GetData<int>("PAYMENT"));
                ...
                player.SendNotification($"Доставка: ~h~~g~+{(level + goshape) * Main.Multipy}$", true);
                return;
            }
