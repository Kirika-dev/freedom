...
namespace NeptuneEVO.Jobs
{
    class Bus : Script
    {
        ...
        public static Vector3 Position = new Vector3(450.5199, -654.3629, 27.26488);

        public static int BusRentCost = 2500;
        ...
        {
            { "Полиция", new Vector3(394.8946, -990.8792, 30.60689) },
            { "Центральная площадь", new Vector3(-528.8386, -328.6082, 36.34783) },
            { "FBI", new Vector3(-1621.519, -532.9644, 35.70459) },
            { "Западная сторона", new Vector3(19.75618, -1533.853, 30.54906) },
            { "Аэропорт", new Vector3(-1032.82, -2723.92, 14.99705) },
            { "Отель аэропорта", new Vector3(-888.1733, -2186.11, 9.900888) },
            { "Автошкола", new Vector3(-663.498, -1244.046, 11.90458) },
            { "Газонокосилка", new Vector3(-1354.111, -43.5153, 52.53339) },
            { "Электростанция", new Vector3(740.4898, 100.5469, 81.29053) },
            { "Такси", new Vector3(918.1192, -188.8451, 74.84467) },
            { "Дальнобойщик", new Vector3(602.8403, -3018.18, 6.131153) },
            { "Восточная сторона", new Vector3(837.6479, -1807.675, 29.10327) },
            { "Инкасатор", new Vector3(807.4769, -1195.802, 27.39124) },
            { "Механик", new Vector3(449.7962, -1249.931, 30.22602) },
            { "Чумаш", new Vector3(-3104.435, 1097.097, 20.59407) },
            { "Палето-Бей", new Vector3(-151.3444, 6211.825, 31.31864) },
            { "Пустынная деревня", new Vector3(1856.94, 3669.111, 34.11074) },
        };
        #endregion
        ...
        public void onResourceStartHandler()
        {
            try
            {
               ...
               /* foreach (var station in BusStations)
                    NAPI.TextLabel.CreateTextLabel($"~w~Остановка\n~g~{station.Key}", station.Value, 30f, 0.4f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);          */

                ColShape Shape = NAPI.ColShape.CreateCylinderColShape(Position, 1F, 2F);
                NAPI.Marker.CreateMarker(27, new Vector3(450.5199, -654.3629, 27.26488) + new Vector3(0, 0, 0.2f), new Vector3(), new Vector3(), 1f, new Color(0, 86, 214, 220), false, 0);
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~w~Взять автобус"), new Vector3(450.5199, -654.3629, 27.26488 + 1.2), 30f, 0.5f, 0, new Color(255, 255, 255));
                Shape.OnEntityEnterColShape += (s, entity) =>
                {
                    try
                    {
                        NAPI.Data.SetEntityData(entity, "INTERACTIONCHECK", 224);
                    }
        ...
        private static void busCheckpointEnterWay(ColShape shape, Player player)
        {
            try
            {
                ...
                if (!BusWays[way][check].IsStop)
                {
                    ...
                    {
                        int costformash = (int)Math.Floor( (2000 + 100 ) * Group.GroupPayAdd[Main.Accounts[player].VipLvl] * Main.oldconfig.PaydayMultiplier);
                        int level = Main.Players[player].LVL > 5 ? 125 : 20 * Main.Players[player].LVL;
                        ...
                        player.SendNotification($"Маршут: ~h~~g~+{costformash + level}$", true);
                    }
                    ...
                    Trigger.PlayerEvent(player, "createCheckpoint", 3, 1, BusWays[way][check].Pos - new Vector3(0, 0, 1.12), 4, 0, color.Red, color.Green, color.Blue, direction);
                    Trigger.PlayerEvent(player, "createWaypoint", BusWays[way][check].Pos.X, BusWays[way][check].Pos.Y);
                    Trigger.PlayerEvent(player, "createWorkBlip", BusWays[way][check].Pos);
                ...
                {
                    if (NAPI.Data.GetEntityData(player, "WORKCHECK") != check) return;
                    Trigger.PlayerEvent(player, "deleteCheckpoint", 3, 0);
        ...
        public void onPlayerEnterVehicleHandler(Player player, Vehicle vehicle, sbyte seatid)
        {
            try
            {
                ...
                if (player.VehicleSeat == 0)
                {
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы не работаете водителем автобуса. Устроиться можно у начальника", 3000);
                        VehicleManager.WarpPlayerOutOfVehicle(player);
                    }
                }
                else
        ...
        {
            new Vector3(461.5898, -646.6069, 27.16157),
            new Vector3(463.5459, -629.2731, 27.37633),
            new Vector3(465.6772, -609.4728, 27.37934),
        };

        public static Vector3 getBusPos()
        {
        ...
        public static void busspawn(Player player)
        {
            var veh = NAPI.Vehicle.CreateVehicle(VehicleHash.Bus, getBusPos(), 174, 1, 1, "BUS");
            ...
            NAPI.Data.SetEntityData(veh, "NUMBER", "BUS");
        ...
        public static void acceptBusRent(Player player)
        {
                ...
                Trigger.PlayerEvent(player, "createCheckpoint", 3, 1, BusWays[way][0].Pos - new Vector3(0, 0, 1.12), 4, 0, 255, 0, 0, BusWays[way][1].Pos - new Vector3(0, 0, 1.12));
                Trigger.PlayerEvent(player, "createWaypoint", BusWays[way][0].Pos.X, BusWays[way][0].Pos.Y);
                Trigger.PlayerEvent(player, "createWorkBlip", BusWays[way][0].Pos);
